# supertools Theme Override

When visual-explainer's workflow says to pick a palette and font pairing, use these supertools tokens instead. Everything else — layout, structure, components, anti-slop rules — stays as visual-explainer prescribes.

## CSS Custom Properties

Replace visual-explainer's `--bg`, `--surface`, `--border`, `--text`, `--accent` variables with supertools's semantic tokens. Include these as `:root` defaults so the file works standalone. When embedded in the supertools UI via `--render-html`, these get overridden by the active theme.
...
## Mapping visual-explainer variables to supertools tokens

When visual-explainer references or templates use these variables, substitute:

| visual-explainer | supertools | Notes |
|-----------------|-------------|-------|
...
Visual-explainer forbids Inter as `--font-body`. supertools uses Inter as its default sans-serif. This is intentional — supertools's identity is defined by its theme tokens, not font novelty. When using this skill, Inter is permitted as the body font because the output is meant to look like part of supertools, not like an independent design piece.
...
When visual-explainer instructs you to set `themeVariables` in Mermaid config, use supertools tokens:
...
supertools handles dark/light via theme classes, not `prefers-color-scheme`. The standalone defaults above are the light theme. When embedded via `--render-html`, the active theme's tokens override automatically — no media query needed in the generated HTML.

For standalone viewing, you may optionally add a `prefers-color-scheme: dark` block with the supertools dark theme values:
...
Visual-explainer defines depth tiers (hero, elevated, default, recessed). Map them using supertools tokens:
