---
name: supertools-visual-explainer
disable-model-invocation: true
description: >
  Generate self-contained HTML visualizations with supertools theming. Use for implementation
  plans, PR explainers, architecture diagrams, data tables, slide decks, and any visual
  explanation of technical concepts. Plans and PR explainers follow supertools's prescriptive
  approach; all other visual content delegates to nicobailon/visual-explainer.
---

# supertools Visual Explainer
...
**Everything else** (architecture diagrams, data tables, slide decks, project recaps, general visual explanations) → Follow the [Visual explainer path](#visual-explainer-path). Delegates to nicobailon/visual-explainer with supertools theme tokens.

## Delivery

Always deliver via supertools's annotation UI. Do NOT use `open` or `xdg-open`.

**Plans/proposals** (user should approve/deny):
```bash
supertools annotate <file> --render-html --gate
```

**Everything else** (informational):
```bash
supertools annotate <file> --render-html
```
...
1. `references/design-system.md` — supertools theme tokens, typography, component patterns
...
4. Read `references/theme-override.md` — supertools tokens replacing Nico's palettes

Follow visual-explainer's structure, component classes (`.ve-card`, `.kpi-card`, `.pipeline`), and anti-slop rules. The only override is the color/typography layer — supertools tokens instead of Nico's custom palettes.
