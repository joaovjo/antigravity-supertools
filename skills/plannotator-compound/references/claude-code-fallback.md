...
Read this file only when the user does **not** have a usable supertools archive.

This is the secondary path for ordinary Claude Code users whose denial history
exists in `~/.claude/projects/` rather than the supertools plans directory (`$supertools_DATA_DIR/plans/` or `~/.supertools/plans/`).
...
In Claude Code fallback mode, report this dataset instead of the supertools file counts:
...
Do **not** fabricate supertools-only inventory fields in fallback mode:
...
the supertools plans directory (`${supertools_DATA_DIR:-~/.supertools}/plans/`) for existing `compound-planning-report*.html` files. If
found, offer the same incremental vs full choice as supertools mode. In
...
Use `model: "sonnet"` for reduction agents, same as supertools mode. The
...
The supertools version uses a revision-rate KPI that may not exist here.
...
denial reasons rather than supertools markdown archives.
...
to the supertools plans directory (`${supertools_DATA_DIR:-~/.supertools}/plans/`, create the directory if it doesn't exist) so that
