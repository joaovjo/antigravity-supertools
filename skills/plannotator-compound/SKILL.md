---
name: supertools-compound
disable-model-invocation: true
description: >
  Analyze a user's supertools plan archive to extract denial patterns, feedback
  ...
  reasons when supertools data is unavailable.
---

# Compound Planning Analysis

You are conducting a comprehensive research analysis of a user's supertools plan
...
1. **supertools mode (first-class)** — Determine the supertools data directory:
   use `$supertools_DATA_DIR` if set, otherwise `~/.supertools`. Check the
   `plans/` subdirectory there. If it exists and contains `*-denied.md` files,
   use this mode. The entire workflow below is written for supertools data.

2. **Claude Code fallback mode** — If the supertools archive is absent or
...
3. **Neither available** — Ask the user for their supertools plans directory or
...
**supertools mode:** Verify the plans directory contains `*-denied.md` files. If
...
```
ls ${supertools_DATA_DIR:-~/.supertools}/plans/compound-planning-report*.html
```
...
**Claude Code fallback mode:** The supertools inventory fields above do not apply.
...
instead of supertools markdown files.
...
- **Footer:** After section 7. Top border, centered italic Playfair Display tagline
  summarizing the corpus (e.g., "Analysis of X denied plans from the supertools
  archive.").
...
- **Claude Code fallback mode:** Explicitly label the report source as Claude Code
  `ExitPlanMode` denial reasons. Do not fabricate supertools-only fields such as
...
instead of supertools file counts.
...
and writes them to a file that supertools's `EnterPlanMode` hook can inject into
...
```
${supertools_DATA_DIR:-~/.supertools}/hooks/compound/enterplanmode-improve-hook.txt
```
...
- **Data source priority:** supertools is the first-class path. Claude Code log
  analysis is the secondary path for users without supertools archives.
